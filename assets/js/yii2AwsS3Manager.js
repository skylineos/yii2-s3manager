...
$(document).ready( function() {
    ...
    $.get('/s3mediamanager/default/get-bucket-object', function(data) {
        ...
        {
            $('#files').append(buildFileRow(bucketObject['/'][file].icon, bucketObject['/'][file].text, bucketObject['/'][file].id, bucketObject['/'][file].modified, convertSize(bucketObject['/'][file].size)));
        }
...
function createJsTree(data)
{
    $('#folderTree').jstree({
        ...
        $.post('/s3mediamanager/default/delete-folder', { 
            ...
            {
                alert('This folder contains objects and therefore cannot be deleted. Please remove the objects before deleting the folder.');
                $.get('/s3mediamanager/default/get-bucket-object', function(data) {
                    var obj = JSON.parse(data);
                    $('#folderTree').jstree(true).settings.core.data = obj.folderObject;
                    $('#folderTree').jstree(true).refresh();
    ...
    init: function() {
        this.on("success", function(file) {
            ...
            $.get('/s3mediamanager/default/get-object?justPath=false&key='+key, function(data) {
                var objectHead = JSON.parse(data);
...
$('#s3mm-object-list').on('click', '.s3mm-delete-object', function(e, data) {
    var r = confirm("Are you sure you would like to delete this file? This cannot be undone.");
